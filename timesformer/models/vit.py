...
class VisionTransformer(nn.Module):
    """ Vision Transformere
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=10000, embed_dim=768, depth=12,
    ...
    '''def forward(self, x):
        b, f, p = x.shape[:3]
        x = rearrange(x, 'b f (h w) c -> (b f) (h w) c', h=self.patch_size[0])
        ...
        x = self.norm(x)
        # return the feature vector of the cls token and the logits
...
class TimeSformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=10000, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
